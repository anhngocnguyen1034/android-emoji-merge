...
import com.anhnn.emoji_merge.data.WordCandidate
...
data class WordUiState(
    val level: Int = 1,
    val word: String = "",
    val candidates: List<WordCandidate> = emptyList(),
    val selected: List<WordCandidate> = emptyList(),
    val answerHexes: List<String> = emptyList(),
...
class WordPictogramViewModel(app: Application) : AndroidViewModel(app) {

    private val prefs = GamePrefs(app)
    private val _state = MutableStateFlow(WordUiState(level = prefs.wordPictogramLevel))
    ...
    private fun loadLevel() {
        ...
        val lv = GameLevels.wordLevel(level)
        if (lv == null) {
            _state.update { it.copy(phase = GamePhase.Playing, word = "", candidates = emptyList()) }
            return
        }
        _state.update {
            it.copy(
    ...
    fun toggle(candidate: WordCandidate) {
        if (_state.value.phase != GamePhase.Playing) return
        val sel = _state.value.selected.toMutableList()
        when {
            sel.contains(candidate) -> sel.remove(candidate)
            sel.size < 2 -> sel.add(candidate)
        }
        _state.update { it.copy(selected = sel) }
    }

    /** Gọi sau khi màn hình đã hiện xong quảng cáo confirm. Đáp án tính theo THỨ TỰ. */
    fun confirm() {
        val s = _state.value
        if (s.selected.size < 2) return
        if (s.selected.map { it.hex } == s.answerHexes) win() else lose()
    }
